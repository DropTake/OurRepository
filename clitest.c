...
#endif
///
int cmd_Hello_Dinar(struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc)
{
    cli_print(cli, "Hello, Dinar!");
...
int read_history(struct cli_def *cli, UNUSED(const char *command), UNUSED(char *argv[]), UNUSED(int argc))
{
   ...
   // поток данных 
   FILE *mf;
   // Переменная, в которую поочередно будут помещаться считываемые строки cli->history[]
   ...
   while (1)
   {
      ...
      estr = fgets (cli->history[i],sizeof(cli->history),mf);
      i++;
      ...
      //Если файл не закончился, и не было ошибки чтения, то продолжаем
   }

   // Закрываем файл
   if ( fclose (mf) == EOF) printf ("error_exit_file\n");
...
{
    if (strcasecmp(username, "1") != 0)
        return CLI_ERROR;
    if (strcasecmp(password, "1") != 0)
        return CLI_ERROR;
    return CLI_OK;
}
...
*/
    ...
    c = cli_register_command(cli, NULL, "cmd_UNPR_MODE_EXEC", cmd_Hello_Dinar, PRIVILEGE_UNPRIVILEGED, MODE_EXEC,
                         "Cmd in UNPRIVILEGED MODE_EXEC");
    cli_register_command(cli, c, "sub_cmd_UNPR_MODE_EXEC", cmd_Hello_Dinar, PRIVILEGE_UNPRIVILEGED, MODE_EXEC,
                         "Sub_Cmd in UNPRIVILEGED MODE_EXEC");
    cli_register_command(cli, NULL, "cmd_PR_MODE_EXEC", cmd_Hello_Dinar, PRIVILEGE_PRIVILEGED, MODE_EXEC,
                         "Cmd in PRIVILEGED MODE_EXEC");
    cli_register_command(cli, NULL, "cmd_PR_MODE_CONFIG", cmd_Hello_Dinar, PRIVILEGE_PRIVILEGED, MODE_CONFIG,
                         "Cmd in PRIVILEGED MODE_CONFIG");
    cli_register_command(cli, NULL, "cmd_PR_ MODE_CONFIG_INT,", cmd_Hello_Dinar, PRIVILEGE_PRIVILEGED,  MODE_CONFIG_INT,
                         "Cmd in PRIVILEGED  MODE_CONFIG_INT");
/////////
